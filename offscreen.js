...
(function () {
  ...
  try {
    chrome.storage.local.get(["neoampEq", "neoampVolume"], function (r) {
      ...
      if (r && typeof r.neoampVolume === "number") {
        volume = clamp(r.neoampVolume, 0, 1);
        applyVolume();
      }
    });
  } catch (_) {}
  ...
  function applyVolume() {
    if (masterNode) masterNode.gain.value = clamp(volume, 0, 1);
  }
  ...
  async function start(streamId) {
    try {
      ...
      if (balanceNode) {
        ...
        balanceNode.connect(masterNode);
      } else {
  ...
  function startPump() {
    var nT = analyser.fftSize,
      nF = analyser.frequencyBinCount;
    ...
    pumpTimer = setInterval(function () {
      ...
      try {
        chrome.runtime
          .sendMessage({ target: "sw", type: "fft", b64: bytesToB64(packed) })
          .catch(function () {});
      } catch (_) {}
    }, 20);
  }
  ...
  function stop() {
    ...
    ctx = stream = src = preampNode = balanceNode = masterNode = analyser = null;
    filters = [];
  }
})();
